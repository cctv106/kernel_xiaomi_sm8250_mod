...
#include <trace/events/ksu_trace.h>
...
void ksu_trace_do_execve_hook(void *data, int *fd, struct filename **filename_ptr, void *argv, void *envp, int *flags)
{
...
void ksu_ksud_init()
{
    register_trace_ksu_do_execve_hook(ksu_trace_do_execve_hook, NULL);
#ifdef CONFIG_KPROBES
	int ret;
...
void ksu_ksud_exit()
{
    unregister_trace_ksu_do_execve_hook(ksu_trace_do_execve_hook, NULL);
#ifdef CONFIG_KPROBES
	unregister_kprobe(&execve_kp);
