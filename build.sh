...
#    curl -LSs "https://raw.githubusercontent.com/Prslc/KernelSU/main/kernel/setup.sh" | bash -s non-gki
  ...
  curl -fsSL "https://raw.githubusercontent.com/SukiSU-Ultra/SukiSU-Ultra/$BRANCH_NAME/kernel/Makefile" -o "KernelSU/kernel/Makefile"
  KSU_MAKEFILE_PATH="KernelSU/kernel/Makefile"
  if [ -f "$KSU_MAKEFILE_PATH" ]; then
      KSU_VERSION_API=$(grep 'KSU_VERSION_API :=' "$KSU_MAKEFILE_PATH" | awk -F':=' '{print $2}' | xargs)
...
Build_AOSP() {
  ...
  echo 1 > out/.version
  export KBUILD_BUILD_VERSION="1" LOCALVERSION="-g92c089fc2d37" KBUILD_BUILD_USER="Chinese" KBUILD_BUILD_HOST="root" KBUILD_BUILD_TIMESTAMP="Wed Jun 5 13:27:08 UTC 2024"
